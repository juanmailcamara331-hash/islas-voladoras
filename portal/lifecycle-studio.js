(function(){
  ...
  ready(function(){
    ...
    Promise.all([fetch('ISL_GAMEDEV_OS_CURRENT.json?ts='+Date.now(),{cache:'no-store'}).then(function(r){return r.json()}),fetch('ISL_SECURITY_OS_CURRENT.json?ts='+Date.now(),{cache:'no-store'}).then(function(r){return r.json()})]).then(function(res){var os=res[0],sec=res[1];
      ...
      var secOpen=(sec.current_findings||[]).filter(function(x){return String(x.status).indexOf('OPEN')===0}).length;
      var secDomains=(sec.threat_domains||[]).map(function(x){return '<article class="lifeCard"><div class="label">'+esc(x.id)+'</div><h3>'+esc(x.name)+'</h3><div class="lifeList">'+(x.controls||[]).map(function(c){return '• '+esc(c)}).join('<br>')+'</div></article>'}).join('');
      var secFind=(sec.current_findings||[]).map(function(x){return '<article class="lifeCard"><div class="label">'+esc(x.severity)+' · '+esc(x.status)+'</div><h3>'+esc(x.id)+'</h3><div class="lifeList">'+esc(x.finding)+'<br><b>Acción:</b> '+esc(x.action)+'</div></article>'}).join('');
      view.innerHTML=
        ...
        '<div class="panel" style="border-color:#7d4949;background:linear-gradient(135deg,#241315,#0b171d)"><div class="topbar" style="margin:0 0 8px"><div><div class="eyebrow" style="color:#ff9aa5">SECURITY SHIELD · ZERO TRUST</div><h2 style="font-size:20px">Coraza transversal</h2></div><span class="status bad">'+secOpen+' ABIERTOS</span></div><div class="sub">MODEL → PREVENT → DETECT → LIMIT → RESPOND → RECOVER → LEARN → RETEST. Seguridad bloquea release si toca credenciales, acceso admin, builds, saves, privacidad o supply chain.</div><div class="lifeGrid" style="margin-top:10px">'+secFind+'</div><details style="margin-top:10px"><summary class="label" style="cursor:pointer">VER DOMINIOS DE DEFENSA</summary><div class="lifeGrid" style="margin-top:9px">'+secDomains+'</div></details></div>'+
      ...
      var ex=document.getElementById('lifeExport');if(ex)ex.onclick=function(){dl({schema:'ISL_GAMEDEV_OPERATIONS_PLAN_V1',generated_at:new Date().toISOString(),source_version:os.version,security_version:sec.version,security_gates:sec.gates,security_stop_ship:sec.stop_ship,recursive_loop:os.recursive_loop,lifecycle:os.lifecycle,quality_gates:os.quality_gates,playtests:os.playtest_program,release_marketing:os.release_marketing,economics:os.economics},'ISL_GAMEDEV_OPERATIONS_PLAN.json')};
