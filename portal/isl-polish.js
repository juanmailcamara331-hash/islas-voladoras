(function(){
  ...
  /* Welcome always appears on a fresh page load; Enter closes it immediately and uses that same trusted click to start media/fullscreen. */
  var welcome=document.getElementById('welcome'),enter=document.getElementById('enterISL');
  if(welcome){welcome.classList.remove('hidden');welcome.style.removeProperty('display');welcome.style.visibility='visible';welcome.style.opacity='1';welcome.style.pointerEvents='auto'}
  if(enter){enter.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();if(welcome){welcome.classList.add('hidden');welcome.style.pointerEvents='none'}gestureBootstrap();requestAnimationFrame(function(){window.scrollTo(0,0)})},true)}
