...
need("juanmailcamara331-hash.github.io/islas-voladoras" not in toml, "public Netlify must not redirect to internal GitHub Pages")
need(Path("netlify-private.toml").exists(), "missing dedicated private Netlify config")
private_toml=Path("netlify-private.toml").read_text(encoding="utf-8")
need('X-Robots-Tag = "noindex' in private_toml, "private host must send noindex header")
need('Cache-Control = "no-store, private"' in private_toml, "private host must disable shared caching")
